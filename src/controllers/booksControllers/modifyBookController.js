  ...
  if (foundBook) {
    ...
    if (newData.authors) {
      const newAuthors = await Promise.all(
        newData.authors.map(async (authorName) => {
    ...
    if (newData.genres) {
      const newGenres = await Promise.all(
        newData.genres.map(async (genreName) => {
    ...
    if (newData.editorials) {
      const newEditorials = await Promise.all(
        newData.editorials.map(async (editorialName) => {
          const [newEditorial] = await editorial.findOrCreate({
    ...
    if (newData.languages) {
      const newLanguages = await Promise.all(
        newData.languages.map(async (languageName) => {
